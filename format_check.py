...
def check_formatting(filepath: str, gold_data: dict) -> bool:
    ...
    if isinstance(gold_data, dict):
        expected_ids = [str(k) for k in gold_data.keys()]
    else:
        # Fallback for list format
        expected_ids = [str(v["id"]) for v in gold_data]
    ...
    if error_lines:
        ...
        print(error_lines)
        print(
            "The above lines are malformatted or have wrong indices. Please make sure each line is a valid json and check that each index appears exactly once.")
        return False
    ...
    if questionable_lines:
        print(
            "Warning: The following lines do not have expected values (1-5). Evaluation can still take place but please check your data.")
